...
public class AjaxEntryAction extends BaseServlet {
	...
	private void deleteSpecificEntryBasedOnGivenSheetKeyAndEntrySeqNumber(HttpServletRequest request, HttpServletResponse response, String baseHref, String[] uriFragments) throws IOException {
		...
		int[] monthYrFragmentsIntArr = ObjectUtils.getMonthYr(uriFragments[2]);
		int entrySequence = ObjectUtils.getIntValue(uriFragments[3], NumUtil.MINUS_ONE);
	...
	private void retrieveEntriesBasedOnGivenSheetKey(HttpServletRequest request, HttpServletResponse response, String baseHref, String[] uriFragments) throws IOException {
		...
		String fullSheetKeyId = request.getUserPrincipal().getName() + "_" + uriFragments[2];
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (("".equals(str2)) || ("Cancel".equalsIgnoreCase(str2))) {
			response.sendRedirect(str1 + "/ajax/list/le.jsp#content");
			return;
		}
		...
		try {
			...
			response.sendRedirect(String.format("%s/ajax/list/le.jsp?sid=%s#content", new Object[] { str1, ((EntryStatusWrapper) localObject).getAssociatedSheet().getKeyStr() }));
